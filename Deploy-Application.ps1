...
Try {
	...
	[string]$appVendor = 'nPower Software'
	[string]$appName = 'SolidWorks PowerSurfacing'
	[string]$appVersion = '7.0'
	[string]$appArch = 'x64'
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.0'
	[string]$appScriptDate = '8/1/2022'
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		## Copy .iss answer files to a temp location to call from the installer since a relative path doesn't seem to work, use /sms to work in SCCM, log in normal location
		New-Item "C:\PowerSurfacingInstall\" -itemType Directory
		Copy-File -Path "$dirFiles\setup.iss" -Destination "C:\PowerSurfacingInstall\"
		Execute-Process -Path "$dirFiles\PowerSurfacing_7_0_SW2022.exe" -Parameters '/s /sms /f1"C:\PowerSurfacingInstall\setup.iss" /f2"C:\Windows\Logs\Software\PowerSurfacingLog.log"' -WaitForMsiExec:$true
		Remove-Folder -Path "C:\PowerSurfacingInstall\"
		...
		## Set registry keys that handle license file and automatic startup
		Set-ItemProperty -Path 'HKLM:\SOFTWARE\SolidWorks\SOLIDWORKS 2022\Addins\{b27796b2-1688-43c2-bcce-091132f52982}' -Name ‘(Default)’ -Value 1
		[scriptblock]$HKCURegistrySettings = {


			Set-RegistryKey -Key 'HKCU\SOFTWARE\PowerSurfacing70ForSolidworks\NodeLockedLicense' -Name 'LicensePath' -Value '\\vmwfs14\lab\LabInstallers\Solidworks PowerSurface' -SID $UserProfile.SID
			}
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		New-Item "C:\PowerSurfacingInstall\" -itemType Directory
		Copy-File -Path "$dirFiles\uninstall.iss" -Destination "C:\PowerSurfacingInstall\"
		Execute-Process -Path "$dirFiles\PowerSurfacing_7_0_SW2022.exe" -Parameters '/s /uninst /sms /f1"C:\PowerSurfacingInstall\uninstall.iss" /f2"C:\Windows\Logs\Software\PowerSurfacingLog.log"' -WaitForMsiExec:$true
		Remove-Folder -Path "C:\PowerSurfacingInstall\"
		...
		[scriptblock]$HKCURegistrySettings = {
			Remove-RegistryKey -Key 'HKCU\SOFTWARE\PowerSurfacing70ForSolidworks\NodeLockedLicense' -Recurse -SID $UserProfile.SID
			}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDKHPE7JJOfJaw2
# GoFz3uzP/8sQRg+TXpydU33U1krz+aCCH8EwggVvMIIEV6ADAgECAhBI/JO0YFWU
...
# AYI3AgELMQ4wDAYKKwYBBAGCNwIBFTAvBgkqhkiG9w0BCQQxIgQ